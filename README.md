...
services:
  ...
  nu-example-scenarios-library:
    ...
    environment:
      ...
      NU_REQUEST_RESPONSE_OPEN_API_SERVICE_URL: "http://designer:8181"
...
(e.g. script `loan.sh` generates requests that will be sent to `${NU_REQUEST_RESPONSE_OPEN_API_SERVICE_URL}/scenario/loan`).
...
sent to `${NU_REQUEST_RESPONSE_OPEN_API_SERVICE_URL}/scenario/loan`). The file contains request payload per line. 
