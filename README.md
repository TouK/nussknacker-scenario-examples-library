...
services:
  ...
  nu-example-scenarios-library:
    ...
    environment:
      ...
      NU_REQUEST_RESPONSE_OPEN_API_SERVICE_ADDRESS: "http://designer:8181"
...
- `NU_REQUEST_RESPONSE_OPEN_API_SERVICE_ADDRESS` - it contains the address of the server which exposes Request-Response 
...
(e.g. script `loan.sh` generates requests that will be sent to `${NU_REQUEST_RESPONSE_OPEN_API_SERVICE_ADDRESS}/scenario/loan`).
...
sent to `${NU_REQUEST_RESPONSE_OPEN_API_SERVICE_ADDRESS}/scenario/loan`). The file contains request payload per line. 
